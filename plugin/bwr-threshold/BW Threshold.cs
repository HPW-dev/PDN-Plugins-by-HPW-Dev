...
#region UICode
DoubleSliderControl g_pre_brightness = 0; // [-1,1] pre brightness
...
unsafe ColorBgra process_color(ColorBgra src) {
  ...
  double g = b2d(src.G);
  double b = b2d(src.B);
  r = brightness(r, g_pre_brightness);
  g = brightness(g, g_pre_brightness);
  b = brightness(b, g_pre_brightness);
  r = gamma(r, g_pre_gamma);
  g = gamma(g, g_pre_gamma);
  b = gamma(b, g_pre_gamma);
  r = contrast(r, g_pre_contrast);
  g = contrast(g, g_pre_contrast);
  b = contrast(b, g_pre_contrast);
